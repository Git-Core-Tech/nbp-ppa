...
//
// TMI1910 fixed-width message parser.
// Field positions and widths are derived from the NBP/Fractals AUTH_REQUEST spec.
// Total message length: 1365 bytes.
...
// Complete TMI1910 field map: name → { pos, len, type }
export const TMI_FIELDS: Record<string, FieldDef> = {
  interface_version:           { pos: 0,    len: 4,  type: 'varchar'    },
  record_type:                 { pos: 4,    len: 2,  type: 'number'     },
  remote_time_sent:            { pos: 6,    len: 16, type: 'number'     }, // YYYYMMDDHHMMSS zero-padded to 16
...
export const TMI_MESSAGE_LENGTH = 1365;

export type ParsedTmi = Record<string, string>;

/**
 * Parse a raw TMI1910 fixed-width string into a key→value map.
 * VARCHAR/TEXTDATE fields are trimmed. NUMBER fields strip leading zeros.
 * NUMBER_2DP fields are converted to decimal strings ("2500.00").
 */
export function parseTmi1910(raw: string): ParsedTmi {
  const result: ParsedTmi = {};
  for (const [name, def] of Object.entries(TMI_FIELDS)) {
...
function extractValue(raw: string, type: FieldDef['type']): string {
  switch (type) {
    ...
    case 'number_2dp': {
      // Stored as integer * 100, e.g. 2500.00 → "00000000000250000"
      const n = parseInt(raw, 10);
