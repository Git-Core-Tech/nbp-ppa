...
const enrichmentBuilder_1 = require("../builders/enrichmentBuilder");
exports.readyChecks = {};
/**
 ...
 */
async function CreateDatabaseManager(config, hooks) {
    const manager = {};
    exports.readyChecks = {};
    ...
    if (config.configuration) {
        await (0, configurationBuilder_1.configurationBuilder)(manager, config.configuration, config.localCacheConfig, hooks?.onConfigLoaded);
    }
    if (config.enrichment) {
        await (0, enrichmentBuilder_1.enrichmentBuilder)(manager, config.enrichment);
    }
...
async function CreateStorageManager(requiredStorages, requireAuth = false, hooks) {
    let config = {};
    ...
    if (!Object.values(config).every((value) => value === undefined)) {
        return { db: (await CreateDatabaseManager(config, hooks)), config };
    }
    else {
