...
exports.createSafeObjectFromEndpoint = createSafeObjectFromEndpoint;
const redis_1 = require("../services/redis");
const redis_config_1 = require("../config/redis.config");
const schemaCompileCache = new Map();
let redisServiceSingleton;
...
const getEmsCompatibleRedisConfigFromEnv = () => {
...
const toPath = (basePath, propName) => (basePath ? `${basePath}.${propName}` : propName);
const inferNodeType = (schemaNode) => {
    if (!isObject(schemaNode)) {
        return 'unknown';
    }
    const schemaType = schemaNode.type;
    if (Array.isArray(schemaType)) {
        const firstType = schemaType.find((item) => typeof item === 'string' && item !== 'null');
...
const compileSchemaPaths = (schema) => {
    const pathTypeMap = new Map();
    const visit = (node, basePath) => {
        if (!isObject(node)) {
            return;
        }
        ...
        if (Array.isArray(node.oneOf)) {
            node.oneOf.forEach((item) => {
                visit(item, basePath);
            });
        }
        if (Array.isArray(node.allOf)) {
            node.allOf.forEach((item) => {
                visit(item, basePath);
            });
        }
    };
...
const parseSchemaBundle = (bundle, endpointPath) => {
    if (!isObject(bundle)) {
...
const coerceLeafValue = (value, expectedType) => {
...
const createSafeProxy = (target, pathTypeMap, basePath) => {
    if (!isObject(target)) {
...
const getOrCreateRedisService = async (providedRedisService) => {
    ...
    let redisConfig;
    try {
        const managerConfig = (0, redis_config_1.validateRedisConfig)(false);
        redisConfig = managerConfig.redisConfig;
    }
    catch (error) {
        const emsCompatibleConfig = getEmsCompatibleRedisConfigFromEnv();
        if (emsCompatibleConfig) {
            redisServiceSingleton = await redis_1.RedisService.create(emsCompatibleConfig);
            return redisServiceSingleton;
        }
        throw error;
    }
    ...
    redisServiceSingleton = await redis_1.RedisService.create(redisConfig);
    return redisServiceSingleton;
};
const getSchemaPathMap = (endpointPath, schema, bypassCompileCache) => {
...
async function createSafeObjectFromEndpoint(endpointPath, payload, options = {}) {
    ...
    if (typeof redisService.getEndpointSchemaBundle === 'function') {
        ...
        try {
            schemaBundle = JSON.parse(cacheText);
        }
        catch {
...
//# sourceMappingURL=safeObjectFromSchema.js.map